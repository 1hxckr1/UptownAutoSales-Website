...
export default function VehicleDetails() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 pt-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-4">Loading vehicle...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !vehicle) {
    return (
      <div className="min-h-screen bg-gray-50 pt-24 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-900 text-xl mb-4">{error || 'Vehicle not found'}</p>
          <Link to="/inventory" className="text-red-600 hover:text-red-700">
            Back to Inventory
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link
          to="/inventory"
          className="inline-flex items-center space-x-2 text-gray-500 hover:text-gray-900 mb-8 transition-colors"
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm">
              ...
              {vehicle.trim && <p className="text-gray-500 text-xl mb-2">{vehicle.trim}</p>}
              {vehicle.stock_number && (
                <p className="text-gray-400 text-sm mb-4">Stock #{vehicle.stock_number}</p>
              )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6 mb-8">
                {vehicle.year > 0 && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <Calendar className="w-6 h-6 text-blue-700 mb-2" />
                    <div className="text-gray-500 text-sm">Year</div>
                    <div className="text-gray-900 font-semibold">{vehicle.year}</div>
                  </div>
                )}
                {vehicle.mileage > 0 && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    ...
                    <div className="text-gray-900 font-semibold">{vehicle.mileage.toLocaleString()} mi</div>
                  </div>
                )}
                {vehicle.fuel_type && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <Fuel className="w-6 h-6 text-blue-700 mb-2" />
                    <div className="text-gray-500 text-sm">Fuel Type</div>
                    <div className="text-gray-900 font-semibold">{vehicle.fuel_type}</div>
                  </div>
                )}
                {(vehicle.color || vehicle.exterior_color) && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <Palette className="w-6 h-6 text-blue-700 mb-2" />
                    <div className="text-gray-500 text-sm">Exterior Color</div>
                    <div className="text-gray-900 font-semibold">{vehicle.color || vehicle.exterior_color}</div>
                  </div>
                )}
                {vehicle.interior_color && (
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <Paintbrush className="w-6 h-6 text-blue-700 mb-2" />
                    <div className="text-gray-500 text-sm">Interior Color</div>
                    <div className="text-gray-900 font-semibold">{vehicle.interior_color}</div>
                  </div>
                )}
              ...
              <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {descriptionParagraphs.map((paragraph, index) => (
                        <p key={index} className="text-gray-600 leading-relaxed whitespace-pre-line">{paragraph}</p>
                      ))}
                    </div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {vehicle.ai_detected_features.confirmed?.map((feature, index) => (
                        <div key={`confirmed-${index}`} className="flex items-center space-x-2 text-gray-600">
                      ...
                      {vehicle.ai_detected_features.suggested?.map((feature, index) => (
                        <div key={`suggested-${index}`} className="flex items-center space-x-2 text-gray-600">
                          <div className="w-2 h-2 rounded-full bg-blue-600"></div>
                          <span>{feature}</span>
                          <span className="text-xs text-blue-700 bg-blue-100 px-2 py-0.5 rounded-full">AI Detected</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {vehicle.body_style && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        ...
                        <span className="text-gray-900 font-semibold">{vehicle.body_style}</span>
                      </div>
                    )}
                    {vehicle.transmission && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">Transmission</span>
                        <span className="text-gray-900 font-semibold">{vehicle.transmission}</span>
                      </div>
                    )}
                    {vehicle.drivetrain && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">Drivetrain</span>
                        <span className="text-gray-900 font-semibold">{vehicle.drivetrain}</span>
                      </div>
                    )}
                    {(vehicle.engine || vehicle.engine_type) && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">Engine</span>
                        <span className="text-gray-900 font-semibold">{vehicle.engine || vehicle.engine_type}</span>
                      </div>
                    )}
                    {(vehicle.color || vehicle.exterior_color) && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">Exterior Color</span>
                        <span className="text-gray-900 font-semibold">{vehicle.color || vehicle.exterior_color}</span>
                      </div>
                    )}
                    {vehicle.interior_color && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">Interior Color</span>
                        <span className="text-gray-900 font-semibold">{vehicle.interior_color}</span>
                      </div>
                    )}
                    {vehicle.mpg?.city != null && typeof vehicle.mpg.city === 'number' && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">MPG City</span>
                        <span className="text-gray-900 font-semibold">{vehicle.mpg.city} mpg</span>
                      </div>
                    )}
                    {vehicle.mpg?.highway != null && typeof vehicle.mpg.highway === 'number' && (
                      <div className="flex justify-between py-2 border-b border-gray-200">
                        <span className="text-gray-500">MPG Highway</span>
                        <span className="text-gray-900 font-semibold">{vehicle.mpg.highway} mpg</span>
                      </div>
                    )}
                    <div className="flex justify-between py-2 border-b border-gray-200 col-span-full">
                      <span className="text-gray-500">VIN</span>
                      <span className="text-gray-900 font-semibold text-sm">{vehicle.vin}</span>
                    </div>
                  </div>
                </div>
                ...
                {vehicle.notes && (
                  <div>
                    ...
                    <p className="text-gray-600 leading-relaxed whitespace-pre-line">{vehicle.notes}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm sticky top-24">
              ...
              <p className="text-gray-500 mt-4 mb-8">Contact us for details</p>
              ...
              <div className="space-y-3 mb-8">
                {dealer?.phone && (
                  <a
                    ...
                    className="flex items-center justify-center space-x-2 w-full px-6 py-4 rounded-lg bg-red-600 text-white font-semibold shadow-lg hover:bg-red-700 transition-all duration-300"
                  >
                ...
                <Link
                  ...
                  className="flex items-center justify-center space-x-2 w-full px-6 py-4 rounded-lg bg-blue-900 text-white font-semibold hover:bg-blue-800 transition-all duration-300"
                >
                ...
                <Link
                  ...
                  className="flex items-center justify-center w-full px-6 py-4 rounded-lg bg-gray-100 border border-gray-200 text-gray-700 font-semibold hover:bg-gray-200 transition-all duration-300"
                >
                  Get Pre-Approved
                </Link>
                <Link
                  ...
                  className="flex items-center justify-center space-x-2 w-full px-6 py-3 rounded-lg bg-gray-50 border border-gray-200 text-gray-600 font-medium hover:bg-gray-100 transition-all duration-300"
                >
                ...
                {isValidVin(vehicle) ? (
                  <button
                    ...
                    className="group relative flex items-center justify-center space-x-2 w-full px-6 py-3.5 rounded-lg bg-orange-50 border border-orange-200 text-orange-700 font-semibold hover:bg-orange-100 transition-all duration-300"
                  >
                  ...
                  <div className="relative group">
                    <button
                      disabled
                      className="flex items-center justify-center space-x-2 w-full px-6 py-3.5 rounded-lg bg-gray-50 border border-gray-200 text-gray-400 font-medium cursor-not-allowed"
                    >
                      <FileText className="w-4 h-4" />
                      <span className="text-sm">CARFAX Report</span>
                    </button>
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-gray-900 rounded-lg text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
              ...
              <div className="pt-8 border-t border-gray-200">
                <h3 className="text-gray-900 font-semibold mb-4">Request Information</h3>
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-300 text-gray-900 placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 text-sm"
                  />
                  <input
                    type="email"
                    placeholder="Email"
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-300 text-gray-900 placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 resize-none text-sm"
                  ></textarea>
                  <button
                    type="submit"
                    disabled={formStatus === 'loading'}
                    className="w-full px-6 py-3 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                  >
